...
function renderManageSupermarkets() {
  ...
  supermarkets.forEach(sm => {
    const isCustom = sm.id.startsWith('sm-custom-') || (!sm.id.match(/^sm-(es-|fr-|it-|de-|pt-|nl-|gb-|us-|jp-|cn-|kr-)/) && !['sm-mercadona','sm-caprabo','sm-carrefour','sm-esclat','sm-bonpreu','sm-lidl','sm-aldi','sm-dia','sm-eroski','sm-consum','sm-alcampo'].includes(sm.id));
    ...
    // Editar
    row.querySelector('.manage-sm-edit').addEventListener('click', () => {
      openSupermarketEdit(sm);
    });

    // Esborrar
    row.querySelector('.manage-sm-delete').addEventListener('click', () => {
      askDeleteSupermarket(sm);
    });
...
function buyShoppingItem(item) {
  ...
  // Obrim el formulari d'afegir producte (és el de sempre del tracker)
  // Necessitem prefillar el nom, emoji
  if (typeof openAddForm === 'function') {
    openAddForm({ name: item.name, emoji: item.emoji });
  } else {
    showToast(t('error'));
  }
}
...
function saveNewProduct() {
  ...
  // Si veníem de la llista de la compra, traiem l'item d'allà
