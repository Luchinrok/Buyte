...
// Cada ubicació té un tipus:
//  - 'normal': fa servir la data de caducitat indicada
//  - 'freezer': fa servir la taula FREEZER_DAYS (temps de congelació)
const DEFAULT_LOCATIONS = [
  { id: 'fridge', emoji: '🧊', nameKey: 'locFridge', type: 'normal' },
  { id: 'freezer', emoji: '❄️', nameKey: 'locFreezer', type: 'freezer' },
  { id: 'pantry', emoji: '🥫', nameKey: 'locPantry', type: 'normal' },
  { id: 'fruit_bowl', emoji: '🍎', nameKey: 'locFruitBowl', type: 'normal' },
  { id: 'counter', emoji: '🏠', nameKey: 'locCounter', type: 'normal' }
];
...
function loadLocations() {
  ...
  // Migració del sistema antic (multiplier) al nou (type)
  let migrated = false;
  locations.forEach(loc => {
    if (!loc.type) {
      // Si tenia multiplier alt (>=4), assumim que era el congelador
      if (loc.multiplier && loc.multiplier >= 4) loc.type = 'freezer';
      else loc.type = 'normal';
...
// Calcula dies segons el tipus d'ubicació
...
function renderLocationsList() {
  ...
  locations.forEach((loc, index) => {
    ...
    item.querySelector('.loc-item-mult').textContent =
      loc.type === 'freezer' ? '❄️ ' + t('storageFreezer') : '🌡️ ' + t('storageNormal');
...
let tempLocType = 'normal';

function openLocationEditor(index) {
  editingLocationIndex = index;
  const isNew = index < 0;
  const loc = isNew ? { emoji: '📍', customName: '', type: 'normal' } : locations[index];
  ...
  renderStorageTypePicker();
  showScreen('location-edit');
}

function renderStorageTypePicker() {
  const container = document.getElementById('storage-type-picker');
  if (!container) return;
  container.innerHTML = '';

  const types = [
    { id: 'normal', emoji: '🌡️', labelKey: 'storageNormal', descKey: 'storageNormalDesc' },
    { id: 'freezer', emoji: '❄️', labelKey: 'storageFreezer', descKey: 'storageFreezerDesc' }
  ];

  types.forEach(typ => {
    const btn = document.createElement('button');
    btn.type = 'button';
    btn.className = 'storage-type-option' + (typ.id === tempLocType ? ' selected' : '');
    btn.innerHTML = `
    ...
    `;
    btn.querySelector('.storage-type-emoji').textContent = typ.emoji;
    btn.querySelector('.storage-type-label').textContent = t(typ.labelKey);
    btn.querySelector('.storage-type-desc').textContent = t(typ.descKey);
...
function saveLocationEdit() {
  ...
  if (editingLocationIndex < 0) {
    ...
    locations[editingLocationIndex].type = tempLocType;
  }
