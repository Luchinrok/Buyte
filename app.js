...
function loadShoppingData() {
  ...
  if (sm) {
    try { supermarkets = JSON.parse(sm); }
    catch(e) { supermarkets = [...DEFAULT_SUPERMARKETS]; }
  } else {
...
function renderSupermarkets() {
  ...
  if (supermarkets.length === 0) {
    ...
    empty.textContent = t('noSupermarkets');
...
function saveSupermarket() {
  ...
  if (editingSupermarket) {
    ...
    const id = 'sm-' + Date.now() + '-' + Math.random().toString(36).slice(2, 6);
    supermarkets.push({ id, name, emoji: selectedSupermarketEmoji });
  }
  saveShoppingData();
  showToast(t('saved'));
  openShoppingList();
...
function askAddToShoppingList(product) {
  ...
  const msg = t('askAddToShopping', product.emoji + ' ' + product.name);
  if (!confirm(msg)) return;

  // Si només hi ha un supermercat, l'afegim directament allà
  ...
  // Si hi ha més d'un, mostrem un selector
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const btnAddSupermarket = document.getElementById('btn-add-supermarket');
  if (btnAddSupermarket) btnAddSupermarket.addEventListener('click', () => openSupermarketEdit(null));
