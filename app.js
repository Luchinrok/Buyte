...
function changeCountry(countryCode) {
  ...
  const existingIds = new Set(supermarkets.map(s => s.id));

  // Marquem com a desactivats els supermercats del país anterior (que no s'hagin afegit manualment)
  supermarkets.forEach(s => {
    if (s.id.startsWith('sm-') && !s.id.startsWith('sm-' + Date.now().toString().slice(0, 4))) {
      // Si l'id segueix el patró del país (no és personalitzat), el desactivem si no és del país nou
      const newCountrySet = new Set(newList.map(n => n.id));
      if (!newCountrySet.has(s.id)) {
        s.enabled = false;
      }
    }
  });

  // Afegim els nous (els que no existien) i en marquem els 4 primers
  newList.forEach((sm, idx) => {
...
function renderShoppingItems() {
  ...
  if (items.length === 0) {
    const empty = document.createElement('p');
    empty.className = 'empty-state';
    empty.textContent = t('noShoppingItems');
    container.appendChild(empty);
    return;
  }
...
function renderSupermarketEmojiPicker() {
  const container = document.getElementById('supermarket-emoji-picker');
  container.innerHTML = '';
  SUPERMARKET_EMOJIS.forEach(e => {
    const btn = document.createElement('button');
    btn.className = 'emoji-option' + (e === selectedSupermarketEmoji ? ' selected' : '');
    btn.textContent = e;
    btn.type = 'button';
    btn.addEventListener('click', () => {
      selectedSupermarketEmoji = e;
      document.querySelectorAll('#supermarket-emoji-picker .emoji-option').forEach(b => b.classList.remove('selected'));
...
function renderShoppingEmojiPicker() {
  const container = document.getElementById('shopping-emoji-picker');
  container.innerHTML = '';
  // Reusem els EMOJIS del producte (que ja existeixen)
  const emojis = (typeof PRODUCT_EMOJIS !== 'undefined') ? PRODUCT_EMOJIS :
    ['🥛','🥚','🧀','🥓','🍞','🥖','🍌','🍎','🍊','🍓','🍅','🥦','🥕','🥒','🌽','🍗','🍖','🐟','🍤','🥗','🥫','🍝','🥖','☕','🍫','🍪','🍰','🥤','🧃','🧊'];
  emojis.forEach(e => {
    const btn = document.createElement('button');
    btn.className = 'emoji-option' + (e === selectedShoppingEmoji ? ' selected' : '');
    btn.textContent = e;
    btn.type = 'button';
    btn.addEventListener('click', () => {
      selectedShoppingEmoji = e;
      document.querySelectorAll('#shopping-emoji-picker .emoji-option').forEach(b => b.classList.remove('selected'));
      btn.classList.add('selected');
    });
    container.appendChild(btn);
  });
...
function handleAction(action) {
  ...
  // Pop-up: si s'ha consumit, oferir afegir a la llista de la compra
  if (action === 'consumed' && consumedProduct) {
...
function openAddForm(prefill) {
  ...
  // Ubicació per defecte: nevera
  selectedLocation = (prefill && prefill.location) || 'fridge';
...
function setupNameAutocomplete() {
  ...
  // Amaga suggeriments en perdre focus (amb un petit delay per permetre clicar)
...
function saveNewProduct() {
  ...
  // Guardem el nom al historial per poder-lo recuperar com a suggeriment
  recordProductInHistory(name, selectedEmoji);
  ...
  products.push({
    ...
    qty: qty,           // Quantitat (1L, 500g, 3 unitats...)
...
function recordProductInHistory(name, emoji) {
  ...
  if (existing) {
    ...
    productHistory.push({ name, emoji: emoji || '🥛', count: 1, lastUsed: Date.now() });
  }
  // Ordenem per ús (els més usats primer)
  productHistory.sort((a, b) => b.count - a.count || b.lastUsed - a.lastUsed);
  // Mantenim només els 50 més usats
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('menu-btn').addEventListener('click', () => openSettings('home'));
  ...
  if (btnAddItem) btnAddItem.addEventListener('click', () => openShoppingItemEdit(null));
