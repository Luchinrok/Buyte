...
const CATEGORY_TO_EMOJI = {
  // Cremes per untar (Nutella, etc.)
  'nutella': '🍫', 'spread': '🍫', 'untable': '🍫', 'patê': '🥫',
  // Làctics
...
// Dies de caducitat per defecte (suposem producte TANCAT a la nevera/rebost)
// Aquests valors reflecteixen la durada dels productes envasats abans d'obrir
const CATEGORY_DEFAULT_DAYS = {
  // Cremes per untar tancades duren mesos
  'nutella': 90, 'spread': 90, 'cocoa': 180,
  // Làctics
  'milk': 7, 'dairy': 7,
  ...
  'butter': 60,
  'egg': 28,
  // Carn fresca (no envasada): pocs dies
  'meat': 3, 'beef': 3, 'pork': 3, 'chicken': 2, 'poultry': 2,
  'bacon': 14, 'sausage': 7,
  // Peix
  'fish': 2, 'seafood': 2,
  // Conserves: mesos a anys
  'canned': 730, 'conserve': 730, 'preserved': 365,
...
const DEFAULT_LOCATIONS = [
  { id: 'fridge', emoji: '🧊', nameKey: 'locFridge', multiplier: 1.0 },
  { id: 'freezer', emoji: '❄️', nameKey: 'locFreezer', multiplier: 6.0 },
  { id: 'pantry', emoji: '🥫', nameKey: 'locPantry', multiplier: 1.0 },
  { id: 'fruit_bowl', emoji: '🍎', nameKey: 'locFruitBowl', multiplier: 0.6 },
  { id: 'counter', emoji: '🏠', nameKey: 'locCounter', multiplier: 0.5 }
];
...
function renderLocationsList() {
  ...
  locations.forEach((loc, index) => {
    ...
    item.innerHTML = `
      ...
      <button class="loc-edit-btn" data-action="edit" data-idx="${index}" aria-label="Edit">✏️</button>
      <button class="loc-edit-btn" data-action="delete" data-idx="${index}" aria-label="Delete">🗑️</button>
    `;
    item.querySelector('.loc-item-emoji').textContent = loc.emoji;
    item.querySelector('.loc-item-name').textContent = getLocationName(loc);
    const multText = loc.multiplier === 1 ? t('locStandard') :
                     loc.multiplier > 1 ? '×' + loc.multiplier + ' ' + t('locLonger') :
                     '×' + loc.multiplier + ' ' + t('locShorter');
    item.querySelector('.loc-item-mult').textContent = multText;
...
function openLocationEditor(index) {
  ...
  const isNew = index < 0;
  const loc = isNew ? { emoji: '📍', customName: '', multiplier: 1.0 } : locations[index];
  ...
  document.getElementById('loc-edit-multiplier').value = loc.multiplier;

  // Botó esborrar només per existents (i no per defecte)
...
function saveLocationEdit() {
  ...
  const multiplier = parseFloat(document.getElementById('loc-edit-multiplier').value);

  if (!name) { showToast(t('needName')); return; }
  if (!multiplier || multiplier <= 0) { showToast(t('invalidMultiplier')); return; }
  ...
  if (editingLocationIndex < 0) {
    ...
    // Edició: NO toquem id ni nameKey, només els camps editables
    locations[editingLocationIndex].emoji = tempLocEmoji;
    locations[editingLocationIndex].customName = name;
    locations[editingLocationIndex].multiplier = multiplier;
  }
...
async function onBarcodeDetected(code) {
  ...
  try {
    ...
    if (data) {
      ...
      setTimeout(() => {
        // IMPORTANT: NO passem 'days' perquè la data de caducitat
        // depèn de cada producte concret, no del codi de barres.
        // L'usuari ha de triar-la sempre manualment.
        openAddForm({ name: data.name, emoji: data.emoji, fromScan: true });
      }, 500);
    } else {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Slider del multiplicador (mostrar valor en directe)
  const multInput = document.getElementById('loc-edit-multiplier');
  const multValue = document.getElementById('loc-edit-multiplier-value');
  if (multInput && multValue) {
    multInput.addEventListener('input', () => {
      multValue.textContent = '×' + parseFloat(multInput.value).toFixed(1);
    });
  }
});
