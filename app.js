...
function getPopularProducts() {
  ...
  // Si hi ha personalitzacions, les fem servir; sinó, les per defecte
  const customRaw = localStorage.getItem('eatmefirst_popular_custom');
  if (customRaw) {
    try {
      const custom = JSON.parse(customRaw);
      // Cada item té id, name, emoji, days, customName (si l'usuari l'ha editat)
      return custom;
    } catch(e) {}
  }
  // Per defecte: del catàleg
...
function getBasicShops() {
  return BASIC_SHOPS.map(s => ({
    ...
    name: t(s.nameKey),
    emoji: s.emoji,
    enabled: false // per defecte deshabilitades, l'usuari les activa si vol
  }));
}
...
function initSupermarketsForCountry(countryCode) {
  ...
  supermarkets = [
    ...list.map((sm, idx) => ({
      ...sm,
      enabled: idx < 4 // Els 4 primers marcats per defecte
    })),
    ...getBasicShops()  // Afegim les botigues bàsiques (deshabilitades)
  ];
...
function changeCountry(countryCode) {
  ...
  // Mantenim els supers personalitzats i les botigues bàsiques (carnisseria, peixateria, etc.)
  const customSupers = supermarkets.filter(s =>
    s.id.startsWith('sm-custom-') || s.id.startsWith('sm-shop-')
  );

  // Si encara no tenim totes les botigues bàsiques, les afegim
  ...
  // Esborrem items dels supers que ja no existiran
  const newIds = new Set([
    ...newList.map(n => n.id),
    ...customSupers.map(s => s.id),
    ...missingBasicShops.map(s => s.id)
  ]);
  shoppingItems = shoppingItems.filter(it => newIds.has(it.supermarketId));

  // Reconstruïm: nous país (4 marcats) + custom + bàsiques que faltaven
  supermarkets = [
...
let viewAllSortMode = 'expiry'; // 'expiry' o 'zone'
...
function addSpecialListToSupermarket(supermarketId) {
  ...
  saveShoppingData();
  showToast('🎉 ' + list.items.length + ' ' + t('itemsAdded'));
  showScreen('shopping');
  renderSupermarkets();
}
  ...
  supermarkets.forEach((sm, idx) => {
    ...
    row.innerHTML = `
      ...
      <div class="manage-sm-arrows">
        <button class="arrow-btn ${isFirst ? 'arrow-disabled' : ''}" data-action="up" data-id="${sm.id}" ${isFirst ? 'disabled' : ''} aria-label="Up">▲</button>
        <button class="arrow-btn ${isLast ? 'arrow-disabled' : ''}" data-action="down" data-id="${sm.id}" ${isLast ? 'disabled' : ''} aria-label="Down">▼</button>
      </div>
      <button class="manage-sm-edit" data-id="${sm.id}" aria-label="Edit">✏️</button>
      <button class="manage-sm-delete" data-id="${sm.id}" aria-label="Delete">✕</button>
    `;
...
function renderSupermarketDots() {
  let dotsContainer = document.getElementById('supermarket-dots');
  if (!dotsContainer) {
    // Si no existeix, no fem res (es crea al HTML)
    return;
  }
...
function setupSupermarketSwipe() {
  ...
  screen.addEventListener('touchstart', (e) => {
    // No agafar el swipe si és sobre un botó
  ...
  screen.addEventListener('touchmove', (e) => {
    ...
    // Si és més vertical que horitzontal, abandona
  ...
  screen.addEventListener('touchend', () => {
    ...
    isSwiping = false;
    const dx = currentX - startX;
    if (Math.abs(dx) < 60) return; // ignora swipes massa curts
    ...
    if (dx < 0 && idx < enabled.length - 1) {
      ...
      openSupermarket(enabled[idx + 1].id);
    } else if (dx > 0 && idx > 0) {
...
function openShoppingItemEdit(item) {
  editingShoppingItem = item; // null = nou
  const isNew = !item;
...
function buyShoppingItem(item) {
  // Anem al formulari "Add product" del tracker amb el nom + emoji prefillat
  // El producte es treu de la llista de la compra DESPRÉS que l'usuari el guardi
  pendingShoppingItemId = item.id;
  pendingShoppingSupermarketId = item.supermarketId; // recordem per tornar després

  if (typeof openAddForm === 'function') {
    openAddForm({ name: item.name, emoji: item.emoji, qty: item.qty });
  } else {
    showToast(t('error'));
  }
}
...
function openAddForm(prefill) {
  ...
  if (!defaultLocation && productName) {
    // 1. Cerca a l'historial si l'usuari ha posat aquest producte abans
...
function saveNewProduct() {
  ...
  // Guardem amb dades enriquides per a l'aprenentatge
...
function guessLocationFromName(name) {
  ...
  // Fruita / Verdura → fruiteria
  const fruitsVeg = ['poma', 'pera', 'plàtan', 'platano', 'banana', 'taronja', 'mandarina', 'maduixa', 'fresa',
    'kiwi', 'pinya', 'mel', 'mango', 'raïm', 'uva', 'cirera', 'cereza', 'préssec', 'melocoton',
    'meló', 'síndria', 'sandia', 'tomàquet', 'tomate', 'enciam', 'lechuga', 'pastanaga', 'zanahoria',
    'pebrot', 'pimiento', 'cogombre', 'pepino', 'ceba', 'cebolla', 'all', 'ajo', 'patata',
    'carbassó', 'calabacin', 'bròquil', 'brocoli', 'coliflor', 'espinac', 'espinacas',
    'apple', 'banana', 'orange', 'strawberry', 'tomato', 'lettuce', 'carrot', 'pepper', 'onion', 'garlic'];
  ...
  // Pasta / Arròs / Llegums / Conserves → rebost
  ...
  return null; // No detectat → no canviem la zona
}
...
function recordProductInHistory(name, emoji, location, days) {
  ...
  // APRENENTATGE: si l'usuari ha afegit aquest producte 2+ vegades, l'incorporem als populars
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // El botó d'engranatge del BiteMe ja no existeix (l'usuari va a config des del launcher)
  ...
  document.querySelectorAll('.back-btn').forEach(b => {
    b.addEventListener('click', () => {
      ...
      // Si tornem a la llista de supers o de la compra, re-renderitzem per veure canvis
...
function renderPopularList() {
  ...
  // Botó "Ordenar alfabèticament"
  const toolBar = document.createElement('div');
  toolBar.className = 'popular-toolbar';
  toolBar.innerHTML = `
    <button class="popular-tool-btn" id="popular-sort-alpha">
      🔤 <span data-i18n="sortAlpha">${t('sortAlpha')}</span>
    </button>
    <button class="popular-tool-btn" id="popular-add-custom">
      ➕ <span data-i18n="addPopular">${t('addPopular')}</span>
    </button>
  `;
  container.appendChild(toolBar);
  document.getElementById('popular-sort-alpha').addEventListener('click', () => {
    sortPopularAlpha();
    renderPopularList();
    showToast('🔤 ' + t('sorted'));
  });
  document.getElementById('popular-add-custom').addEventListener('click', addCustomPopular);
