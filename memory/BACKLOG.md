...
- **Despeses i altres** — ~~cost mitjà cistella~~ **✅ FET v1 (11/06, `553ea3f`)**, ~~despesa per categoria~~ **✅ FET (11/06, `6f70cc1`)**, ~~pressupost mensual~~ **✅ FET v1 (11/06, `abd37ff`)**, planificador setmanal de receptes **🚧 Fase 1 (vegeu sota)**, recordatoris contextuals, revisar sistema d'Espais.
  - 🚧 **Planificador setmanal de receptes (CookMe)** — **Fase 1 FETA (12/06, `6d00f06`)**: `js/meal-planner.js` amb template setmanal `{dia:{slot:recipeId}}` a `eatmefirst_meal_plan` (sincronitzat, patró `categoryOrderBySuper`), pantalla `#screen-meal-planner` (7 dies × 3 slots esmorzar/dinar/sopar), assignar/treure via modal selector (llista completa + cerca), format alineat amb "Afegir recepta". **Falta Fase 2**: botó "🛒 Generar llista de la compra" → agregar els ingredients (required) de totes les receptes planificades, fusionar per clau canònica i afegir a BuyMe reusant `addItemsToShop`/`addToShoppingList`/`cookmeSameProduct` (+ selector de supermercat com `showIngredientPicker`).
