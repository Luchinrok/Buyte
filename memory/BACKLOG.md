...
- 🟡 **[NETEJA] Eliminar els selectors orfes manual/codi de barres** — `#screen-add-choice` (EatMe) i `#screen-shopping-add-choice` (BuyMe) ja no es fan servir des que "Afegir producte" obre el formulari manual directament (`a5032e9`). Treure el markup d'`index.html` i els handlers associats a `app.js` (`choice-scan`/`choice-manual`, `shop-choice-scan`/`shop-choice-manual`), comprovant que res més no hi navega (cap `showScreen('add-choice')`/`showScreen('shopping-add-choice')` ni `data-back` que hi apunti que segueixi viu).
