...
- 🟡 **Matching ingredient↔catàleg robust (plurals -os/irregulars + i18n)** (06/06). El lookup del CookMe→BuyMe i el `matchIngredient` usen `cookmeStem`: cobreix plurals catalans regulars (-a↔-es, masculí -s: ou↔ous, mongeta↔mongetes) però NO -os/irregulars (peix↔peixos, -ç→-ços) ni altres idiomes (en: tooth↔teeth; es). Els misses degraden suau (qty 1 sense content). Solució futura: capa d'àlies/sinònims per staple al catàleg (generalitzar el mapa de sinònims de pasta) + irregulars a mà; per i18n, àlies per idioma o referenciar l'id del catàleg en lloc del nom lliure. App avui CA-only en execució → i18n diferit.
