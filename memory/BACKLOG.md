...
- 🟡 **Consistència "mateix producte" a l'EatMe** — aplicar `cookmeSameProduct` a `findExistingAtHome` ("ja en tens a casa") i a l'agregació de productes de l'EatMe, perquè variants singular/plural es reconeguin com un sol producte amb els seus lots. ⚠️ CURA: toca el model central (estoc/lots/dates de caducitat), risc de sobre-fusió → **investigar abans de tocar**.
...
- 🟢 **[SEGÜENT TASCA] Prefill de compra canònic** — en comprar un item del BuyMe amb nom plural d'un popular (ex. "Albergínies" / catàleg "Albergínia"), `_buildShoppingPrefill` (buyme.js ~1876) recupera preu/dies/ubicació/pes per nom EXACTE → falla i demana preu/pes/zona. Fix: afegir el fallback canònic `cookmeSameProduct` (com a `getEstimatedItemCost` / `_buildShoppingItemRow`), amb guarda `typeof cookmeSameProduct==='function'` + fallback exacte.
...
- 🟢 **[CONTINGUT] Completar POPULAR_PRODUCTS amb tots els ingredients del CookMe** que encara no hi son, especialment els compostos/especifics que `cookmeSameProduct` (correctament) NO fusiona amb el base — ex. "pa torrat", "formatge feta". Afegir-los com a entrada propia (noms ca+en, price/weight/days/location, noExpiry si escau) perque en comprar-los heretin dades i no demanin preu/pes/zona. Metode: reusar l'extraccio `cookmeCanonTokens` per llistar ingredients distints i creuar amb el cataleg (com a l'expansio de 44 entrades, `2db9484`).
