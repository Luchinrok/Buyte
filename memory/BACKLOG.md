...
> **Última sincronització: 2026-06-06** (resolts fins 02/06 marcats amb ✅ + hash; fil packs/preu COMPLET `4f95e8f`/`fd5c5b5`/`e500fc0`; polish formularis + centrat LLINDAR `9f03282`; bug agregat→BuyMe + format/centrat modal `45ea959`). **03/06**: tancats com a obsolets/acceptats dos ítems — catàleg contables/preu-unitat (cobert pel fil de packs) + bug visual checkbox multi-lot (acceptat detall menor); ✅ sufix " u" render-only; refactor de lots E/F/G/final APARCAT; ✅ matcher de Cuinat per nom (falsos positius emoji eliminats); + idea importar receptes per fitxer. **05-06/06**: ✅ **Cuinat v2 "He cuinat" COMPLET** (Fases 0–2b-2b: qty estructurades · matcher→producte · cervell+executor FEFO conscient del content · modal+botó); 🔴 obert **fix arrel CookMe→BuyMe** (qty de cuina escrita com a qty de compra) que desbloqueja el descompte automàtic del Cuinat.
...
- 🔴 **PRIORITAT ALTA — Fix arrel CookMe→BuyMe**: `addItemsToShop`/`addMissingToBuyMe` (cookme.js ~1402) escriuen la **qty de CUINA** de la recepta com a qty de compra, **sense weight/content**. En comprar, `_buildLotFromNewProduct` (biteme.js:1187) no la parseja → lot en **`percent` sense unit/content** → al Cuinat queda **`'manual'`** (no es descompta sol). Fix: escriure la **unitat de COMPRA** (qty 1 + content del popular: ous→"12u", mel→el seu envàs), descartant la qty de cuina. Desbloqueja el descompte automàtic. Detall a "Bugs / millores acumulats".
...
- **Refactor de `calculateRecipeMatch`** (falsos positius) — PENDENT. Nota: la Fase 1 del Cuinat va afegir `findProductForIngredient` però amb canvi de comportament ZERO i match al PRIMER resultat (MVP); els falsos positius NO s'han abordat.
