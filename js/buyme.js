...
function _renderShopPageItems(smId, listEl, mode) {
  ...
  // Sempre rendering agrupat per categoria. El toggle entre Cronològic
  // i Per categoria es va eliminar perquè el mode Cronològic disparava
  // un bug de WebKit a iOS amb cadenes llargues d'elements germans i
  // user-select:none (3 zones de scroll asimètric). Categoria, amb
  // .category-section-header trencant la cadena, no té el bug i és
  // la vista més útil. Si el sistema de categories no està disponible,
  // caiem a un render plà sense agrupació (degradació graceful).
